...
// Verifica se está sendo executado como template inicializador
const projectNameArg = process.argv[2];
const isTemplateInit = projectNameArg && !projectNameArg.startsWith('-');
...
// Função para copiar template e criar novo projeto
function initializeProject(projectName) {
    ...
    try {
        ...
        fs.cpSync(templatePath, targetPath, {
            recursive: true,
            filter: (src) => {
                // Ignora node_modules, .git e outros arquivos desnecessários
                const relativePath = path.relative(templatePath, src);
                return !relativePath.match(/^(node_modules|\.git|dist|\.vscode)/);
            }
        });
...
// Se foi passado um nome como argumento, executa modo template
if (isTemplateInit) {
    initializeProject(projectNameArg);
} else {
