...
export function getUserRoles(user: any): string[] {
    ...
    // Formato 1: userRoles direto no user (após processamento no authService)
    if (Array.isArray(user.userRoles)) {
        return user.userRoles
    }
    
    // Formato 2: userRoles no profile
    if (Array.isArray(user?.profile?.userRoles)) {
        return user.profile.userRoles
    }
    
    // Formato 3: roles no profile
    if (Array.isArray(user?.profile?.roles)) {
        return user.profile.roles
    }
    
    // Formato 4: resource_access[client_id].roles (padrão Keycloak)
    const clientId = import.meta.env.VITE_APP_CLIENT_ID || "react-app"
    if (user?.profile?.resource_access?.[clientId]?.roles) {
        return user.profile.resource_access[clientId].roles
    }
    
    // Formato 5: realm_access.roles (roles do realm)
