...
import {
    ...
    formatDate,
    formatHour
} from "../previsao-tempo-service"
import type { WeatherDetail } from "../previsao-tempo-types"
...
export function PrevisaoTempoDetailPage() {
    ...
    async function loadWeather(lat: number, lon: number) {
        ...
        setError(null)
        try {
            const data = await getWeatherDetail(lat, lon)
            if (locationData) {
                data.location = locationData
            }
            setWeather(data)
        } catch (err) {
            console.error("[PrevisãoTempo] Erro ao carregar previsão:", err)
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight">
                            {weather.location.name || "Localidade"}
                        </h1>
                        <p className="text-muted-foreground mt-1">
                            {weather.location.country && `${weather.location.country} • `}
                            Atualizado em {formatHour(weather.current.time)}
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span className="text-lg font-semibold">
                                    {Math.round(weather.current.winddirection)}°
                                </span>
                            </div>
                        </CardContent>
                    </Card>
                                    ...
                                    <div className="flex items-center gap-3">
                                        <div className="w-48 bg-muted rounded-full h-2">
                                            <div
                                                ...
                                                style={{
                                                    width: `${Math.min(100, (weather.daily.precipitation_sum[index] / 50) * 100)}%`
                                                }}
                                            />
                                        </div>
                                        <span className="text-sm font-semibold w-16 text-right">
                                            {weather.daily.precipitation_sum[index].toFixed(1)} mm
                                        </span>
                                    </div>
                                </div>
                            ))}
